...
H.get_matched_ranges_plugin = function(captures)
  ...
  return vim.tbl_map(
    -- Prefer range from metadata (allows customizing range in query)
    function(match) return (match.metadata or {}).range and match.metadata.range or { match.node:range() } end,
  ...
  for _, tree in ipairs(parser:trees()) do
    for capture_id, node, metadata in query:iter_captures(tree:root(), 0) do
      if capture_is_requested[capture_id] then
        ...
        table.insert(res, (metadata or {}).range and metadata.range or { node:range() })
      end
    end
  end
  return res
